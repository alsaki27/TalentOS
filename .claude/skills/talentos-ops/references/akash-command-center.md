...
Join `application_resume_versions` (`ats_score`, `truth_score` — **0–10 scale**; `one_page_fit_score` is a *separate* **0–100 scale**, `contentUtilization*100` — none of these are the 0–100 match-score scale either) through `application_packets.final_resume_version_id` to `application_packets.reviewed_by`/`approved_by`/`sent_by` — that's the person accountable for the packet, not `application_resume_versions.created_by` (frequently the system/AI actor for the first draft). "Low truth score" means `truth_score < 7.0`, not `< 0.7` — see `scripts/ae_tailoring_quality_report.sql`.
