...
This skill assumes narrowly-scoped, read-only-by-default database access reached through a secret store — never a credential typed into a prompt. If no live database connection is configured in this environment, say so plainly and fall back to reference-query output (dry-run SQL + explanation) rather than fabricating results as if a query ran.
