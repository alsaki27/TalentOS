...
async function fix() {
  try {
    await sql`UPDATE application_ai_workflows SET base_resume_id = NULL WHERE base_resume_id NOT IN (SELECT id FROM base_resumes)`;
    console.log("Cleared invalid base_resume_ids");
