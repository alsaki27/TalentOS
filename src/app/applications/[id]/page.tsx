...
export default function ApplicationDetailPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    try {
      const [a, e, c, x] = await Promise.all([
      ...
      const exportData = x.ok ? await x.json() : {}; setExports(exportData.exports || []);
  ...
  return <main className="page-shell" style={{ maxWidth: 1200 }}>
    ...
    <section className="card" style={{ marginTop: 24 }}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 16 }}><Info label="Applied" value={app.applied_at ? new Date(app.applied_at).toLocaleString() : "Not recorded"} /><Info label="Owner" value={app.assigned_to || "Unassigned"} /><Info label="Tailored resume" value={app.tailored_resume_version_id ? app.tailored_resume_version_id.slice(0, 12) : "Not linked"} /><Info label="Application ID" value={params.id} /></div>{app.tailored_resume_version_id && <a className="btn-primary btn-sm" style={{ display: "inline-block", marginTop: 18 }} href={`/falood/studio/application/${app.tailored_resume_version_id}`}>Open tailored resume</a>}</section>
