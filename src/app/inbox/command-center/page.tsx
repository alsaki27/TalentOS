...
type Data = { upcoming: any[]; responses: any[]; history: any[]; signals: any[]; generatedAt: string };
...
export default function InboxCommandCenterPage() {
  ...
  return <main className="page-shell" style={{ maxWidth: 1250 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 12, marginTop: 20 }}>
      {[['Upcoming interviews', data.upcoming.length], ['Replies waiting', waiting], ['Candidates waiting', data.responses.length], ['Recent interview signals', data.signals.length]].map(([label, value]) => <section className="card" key={String(label)}><small className="muted">{label}</small><h2 style={{ margin: "8px 0 0" }}>{value}</h2></section>)}
    </div>
    <section className="card" style={{ marginTop: 20 }}><h2>Upcoming interviews · next 30 days</h2>{data.upcoming.length === 0 ? <p className="muted">No scheduled interviews found.</p> : data.upcoming.map(x => <div key={x.id} style={{ borderTop: "1px solid var(--border)", padding: "12px 0", display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}><div><strong>{x.candidate_name}</strong> · {x.round_name}<div className="muted">{x.job_title || "Job unavailable"}{x.company_name ? ` · ${x.company_name}` : ""} · {date(x.scheduled_at)}</div></div><span className="badge">{x.status || "scheduled"}</span></div>)}</section>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(360px,1fr))", gap: 16, marginTop: 16 }}>
      <section className="card"><h2>Replies waiting</h2>{data.responses.length === 0 ? <p className="muted">No unhandled inbound replies.</p> : data.responses.map(x => <div key={x.candidate_id} style={{ borderTop: "1px solid var(--border)", padding: "12px 0" }}><strong>{x.candidate_name}</strong><span className="badge" style={{ marginLeft: 8, color: "#b45309" }}>{x.waiting_count} waiting</span><p className="muted" style={{ margin: "5px 0 0" }}>Oldest: {date(x.oldest_waiting_at)}</p></div>)}</section>
