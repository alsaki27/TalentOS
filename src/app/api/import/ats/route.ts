...
// POST -> pull live jobs from a company's public Greenhouse/Lever/Ashby job board
// (no auth, no scraping) and bulk insert new ones into the jobs table.
...
export async function POST(req: NextRequest) {
  ...
  if (!PROVIDERS.includes(provider)) {
    return NextResponse.json({ error: "provider must be one of: greenhouse, lever, ashby" }, { status: 400 });
  }
  if (!token) {
    return NextResponse.json({ error: "token (company board slug) is required" }, { status: 400 });
  }
