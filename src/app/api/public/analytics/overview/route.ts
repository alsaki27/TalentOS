...
export async function GET(req: NextRequest) {
  ...
  const jobs = await query('SELECT id, source, company_id FROM jobs', []);
  const allApplications = await query('SELECT id, status, job_id, priority, review_status FROM applications', []);
