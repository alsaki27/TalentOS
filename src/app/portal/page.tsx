...
import { Briefcase, Activity, CalendarCheck2, Award, FileCheck2, ShieldCheck, type LucideIcon } from "lucide-react";
...
// ARCHIVED 2026-09-07 — the "Email privacy controls" card (below, in the
// Account section) is hidden pending a future redesign of this section.
// This type backed that card's state; kept alongside the card itself so a
// restore is a single uncomment. Not currently referenced.
...
export default function PortalDashboardPage() {
  ...
  // ARCHIVED 2026-09-07 — email privacy controls state. Fully working, just
  // not rendered - see the matching JSX block and effect below for the
  // restore note.
  ...
  useEffect(() => {
    ...
    // ARCHIVED 2026-09-07 — see the matching JSX/state block below.
  ...
  // uncomment this block, the state above, the fetch effect above, and the
  // JSX card at the bottom of the Account section.
  ...
  return (
    <PortalShell candidateName={data.name} pageTitle="Overview" onSignOut={logout}>
      <div id="overview" className="portal-hero">
        ...
        <div className="portal-hero-chips">
          <div className="portal-hero-chip"><strong>{data.summary.activeApplications}</strong><span>Active</span></div>
          <div className="portal-hero-chip"><strong>{data.summary.interviews}</strong><span>Interviews</span></div>
          <div className="portal-hero-chip"><strong>{data.summary.offers}</strong><span>Offers</span></div>
        </div>
      </div>
      ...
      <div id="account" className="portal-section">
        ...
        {/* ARCHIVED 2026-09-07 — "Email privacy controls" card, hidden pending
            ...
            state (emailPaused, retentionDays, gmailStatus), handlers
            (updateEmailPrivacy, deleteEmailHistory) and the gmail/privacy
            fetch effect, all marked ARCHIVED above in this same file.
