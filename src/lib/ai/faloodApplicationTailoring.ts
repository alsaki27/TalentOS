...
function buildSuggestPrompt(ctx: TailoringContext): string {
  return [
    ...
    "confidenceScore guide — anchor to these bands, do not eyeball it:",
    "  100: reserved ONLY for inserting an approved keyword verbatim, word-for-word, into existing text with zero rewording. Before assigning 100, check that suggestedText differs from originalText by nothing but the literal keyword insertion.",
    "  90-99: minor rewording of an existing sentence to work in an approved keyword, fully supported by the evidence bank.",
    "  70-89: a suggestion that restructures a sentence, changes emphasis, or reorders content — some judgment call is involved, even if evidence-backed.",
    "  50-69: new phrasing that is inferred from the evidence bank rather than a direct restatement of it.",
    "  Below 50: speculative or touches a keyword with weak/missing evidence — flag it low so it doesn't slip through automated \"accept all high-confidence\" actions.",
    "  Most batches of suggestions should show a real spread across these bands, not cluster at 90+ — if every suggestion you're about to return scores 90+, re-check each one against the band definitions above before responding.",
    ...
    "targetBlockId rules: ALWAYS use a real block id that exists in the resume content JSON above (e.g. the experience entry's own id, or the skill category's id) — never a placeholder label like \"Role 1\". For a brand-new bullet being added to an existing role (not editing one that already exists), set targetBlockId to that role's existing block id, and set originalText to an empty string \"\" (not null, not omitted) to signal this is a net-new addition rather than an edit.",
