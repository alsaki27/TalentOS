...
function readHeader(baseContent: any): ResumeDocumentHeader {
  ...
  // Fallbacks for older base-resume content variants
  return {
...
function mapSkills(skills: string[], baseContent: any) {
  // Group into a single "Skills" group by default. If the base resume already
  // had categorized skills, preserve empty groups to avoid losing structure,
  // but populate from the agent's flat list so ATS keywords land idiomatically.
  const groups: { id: string; title: string; skills: string[] }[] = [];
  if (skills.length > 0) {
    groups.push({ id: uid("skg"), title: "Skills", skills: [...skills] });
  }
  // Preserve any pre-existing skill groups from the base resume that the agent
  // did not explicitly empty out (keeps the studio's skills editor meaningful).
...
function mapCertifications(certs: string[], baseContent: any) {
  // Agent emits flat string[]; preserve structured issuer/date from the base
  // resume when the cert name matches, so we don't strip provenance.
...
/**
 * Build a studio-renderable ResumeDocument by overlaying the agent's
 * FinalResumeV1 output on the candidate's base resume content (preserving the
 * contact header and any provenance the agent schema doesn't carry).
 */
