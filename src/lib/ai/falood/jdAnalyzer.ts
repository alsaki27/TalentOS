...
- "atsKeywords": every term that's likely scanned for, including ones already captured above (e.g., "Bachelor's degree", "5+ years", "Python") - this list can overlap with requiredSkills/tools, it exists as one combined list for a simple "does this resume contain these strings" check. Use the JD's exact phrasing, spelling, and casing (matching is verbatim), keep multi-word phrases intact ("fiber network design", not "fiber" + "design"), and when the JD uses both an acronym and its spelled-out form - or clearly means both - include BOTH as separate entries (e.g., "OSP" and "Outside Plant"), since ATS systems frequently match only one form.
