...
export async function upsertTargetJobByCandidateAndJob(
  ...
  if (isNeon()) {
    ...
    // target_jobs has no updated_at column (unlike most other tables in this schema) —
    // don't append it here, it breaks the DO UPDATE SET clause at parse time even on
    // a fresh insert (Postgres validates the target list regardless of conflict path).
    const updateClause =
      updateCols.length > 0
...
export async function updateTargetJob(id: string, updates: Record<string, unknown>): Promise<any> {
  if (isNeon()) {
    ...
    values.push(id);
    // target_jobs has no updated_at column — see upsertTargetJobByCandidateAndJob above.
