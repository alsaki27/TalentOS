# Candidate self-login dashboard — design doc (not implemented)
...
backend) repos. Everything else in that comparison favored this app — see ROADMAP.md's
"Done" section for the full reasoning — but their `etl` module has one idea genuinely
...
`/portal/<token>` — a public, no-login, read-only page. Each candidate has a
...
pipeline tickets are never exposed. No password, no account, no session — knowledge of the
...
candidate-specific UI beyond two read endpoints in the API — the value here is the
...
1. **A new role or role-like concept.** Today `UserRole` is exactly
   `admin | manager | application_engineer | recruiter` (`src/lib/auth.ts`) — all internal
   ...
   - keep `profiles` staff-only and add a parallel, smaller auth path specifically for
     ...
     Auth users) — more isolated, more code, but zero risk of a staff-route accidentally
...
2. **A signup/invite flow.** Candidates don't self-register today (an internal recruiter
   ...
   with a one-time link to set a password — turning the existing `portal_token` into a
...
3. **Session handling separate from `src/middleware.ts`.** That file's session check is
   ...
   cookie name and its own verification path through middleware — additive, not a rewrite,
...
4. **Decide what happens to `/portal/<token>` during/after this.** Three real options, not
   a forced choice:
   - **Replace it** — every candidate must set up a login; the token link stops working.
     Cleanest long-term, but a breaking change for anyone relying on an existing shared
     link.
   - **Supplement it** — both exist; recruiters choose per-candidate whether to invite them
     to a real login or just hand them the token link. More flexible, more surface area to
     maintain indefinitely.
   - **Migrate gradually** — token links keep working for now, login is opt-in, revisit
     full replacement once adoption data exists.

## Why this wasn't just built

Three of the four items above are decisions, not implementation details — the role model
...
Parallel candidate-session path (option 2 in §1), invite-flow on top of the existing
`portal_token` mechanism rather than replacing it immediately (gradual migration, §4), and
build it as a new, isolated set of files (`src/lib/candidateAuth.ts`,
`src/app/api/candidate-auth/*`) rather than extending `src/lib/auth.ts` — keeps the
