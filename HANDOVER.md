...
1. **`/` (this Next.js app)** — the live, working product. Next.js 14 App Router, plain
...
2. **`/backend`** — a NestJS + TypeORM service, started by the team to migrate off
   Supabase (Postgres stays, but the app layer moves to NestJS, auth moves to Clerk,
   storage moves to SharePoint). **Explicit decision: keep both for now** — this is not a
...
Don't assume code in one implies the other is current — they're maintained by different
...
- **Frontend** (`/`): `npx tsc --noEmit` clean, `npm run build` succeeds (60+ routes),
  `npx supabase migration list` — local and remote fully in sync, nothing pending.
- **Backend** (`/backend`): `npm run typecheck` and `npm run build` both clean. Found and
  ...
  to the root `exclude` array — if you ever see that error again, check this didn't get
  reverted.
- RLS is enabled on every Supabase table, with **zero policies** anywhere. Intentional —
...
|---|---|---|---|
| `SUPABASE_URL` | Everything | ✅ set | Project URL from Supabase dashboard → Settings → API. |
| `SUPABASE_SERVICE_ROLE_KEY` | Everything | ✅ set | Full DB access, bypasses RLS. Never expose client-side, never commit. |
| `NVIDIA_API_KEY` / `NVIDIA_MODEL` | `/chat`, AI digest | ✅ set | Live-tested. See "AI provider" below for its known limitation. |
| `SUPABASE_ANON_KEY` | Login (`/api/auth/login`) | ❌ not set | Falls back to the service-role key if absent — login still works, but set this for real use. |
| `ANTHROPIC_API_KEY` / `ANTHROPIC_MODEL` | `/chat`, AI digest (preferred provider) | ❌ not set | Preferred over NVIDIA automatically once set — see "AI provider" below for why. |
| `AI_PROVIDER` | optional override | ❌ not set | `anthropic` or `nvidia`, forces a choice when both are set. |
| `CRON_SECRET` | Scheduled jobs (`/api/cron/*`) | ❌ not set | **Without this, all three Vercel Cron jobs (import-sources, backup, digest) 401 silently, every day, forever.** Highest-impact missing config for a real deploy. |
| `USAJOBS_API_KEY` / `USAJOBS_USER_AGENT` | USAJobs import | ❌ not set | Free key from developer.usajobs.gov. |
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `GOOGLE_OAUTH_REDIRECT_URI` | Gmail integration | ❌ not set | See `docs/integrations.md` for the full OAuth setup. |
| `TEAMS_TALENT_OS_WEBHOOK_URL` | Outbound Teams notifications | ❌ not set | No-ops cleanly (not an error) when absent. |
| `TALENT_OS_WEBHOOK_SECRET` | Inbound webhook | ❌ not set | Fails closed (401) without it. |
| `CRAWLER_API_KEY` | Job crawler ingestion (`/api/integrations/crawler/jobs`, `/heartbeat`) | ❌ not set | Bearer shared secret for an external crawler bot. Live-tested with a temporary local value — dedup, auth, and real-time push all confirmed working; reverted before handover. |
| `RESUME_STORAGE_PROVIDER` | Resume upload backend | ❌ not set (defaults to `supabase`) | **You'll be setting this to `sharepoint`** — see the dedicated section below before you do. |
| `MS_CLIENT_ID` / `MS_CLIENT_SECRET` / `MS_TENANT_ID` / `SHAREPOINT_SITE_ID` / `SHAREPOINT_DRIVE_FOLDER` | SharePoint resume storage | ❌ not set | See "Switching resume storage to SharePoint" below — required reading before flipping `RESUME_STORAGE_PROVIDER`. |

**No `.env.example` file exists for the frontend.** `backend/.env.example` does exist —
see its own section below.

## Switching resume storage to SharePoint — what you actually need
...
actual SharePoint tenant** — there was no Microsoft 365 tenant available in this
environment to test against. Before relying on it:

1. **Register an Azure AD app** (Azure Portal → App registrations → New registration).
   Note the **Application (client) ID** and **Directory (tenant) ID** — these become
   `MS_CLIENT_ID` and `MS_TENANT_ID`.
2. **Create a client secret** under that app (Certificates & secrets) — this becomes
   `MS_CLIENT_SECRET`. It expires (you choose 6/12/24 months) — whoever owns this needs a
...
3. **Grant Microsoft Graph application permissions**: `Sites.ReadWrite.All` (or
   `Files.ReadWrite.All` if scoping to one drive). This requires **admin consent** — your
   M365 tenant admin has to click "Grant admin consent" in the Azure portal once.
4. **Find your `SHAREPOINT_SITE_ID`** — the Graph API site identifier (not the site URL).
...
5. Set `RESUME_STORAGE_PROVIDER=sharepoint` plus the four vars above. **Test with one real
   upload through `/candidates/[id]` before trusting it in production** — this is the one
...
6. **Known limitation, not yet handled**: `src/lib/storage.ts`'s `deleteStorageFile()` only
   knows how to clean up Supabase Storage URLs — it silently no-ops on a SharePoint URL
...
## AI provider — the one nuance worth understanding before touching `/chat`
...
tool result to consume) — that's why the daily digest uses single-shot, not tool-calling.
Two mitigations exist (penalty params + a `looksDegenerate()` fallback) but they reduce
the failure rate, not eliminate it. **Prefer Anthropic once a key exists** —
...
- **Auth is sound.** `src/middleware.ts` blocks every route except an explicit allowlist
  ...
  `getCurrentUserContext()` and 401 without a session — not actually open.
...
- **Role gating is partial, by design-decision-not-yet-made, not by oversight.** Same as
  before — see ROADMAP.md's "Next up" list.
- **RLS has no policies** — not a vulnerability today (nothing uses the anon key against
...
1. **Every entity carries `is_deleted`/`deleted_at` via `BaseEntity` — no live Supabase
   ...
   There's no `src/database/migrations` folder yet — only `TYPEORM_SYNCHRONIZE`, which the
...
2. **Auth provider is switching from Supabase Auth to Clerk**, and `profiles`' primary key
   shape is changing (`user_id` = `auth.users.id` → a new generated `id` + separate
...
3. **Most of the app isn't ported yet**, by the team's own list in `backend/MIGRATION.md`:
   ...
   ingestion (built and live-tested on the frontend this session — see README.md), AI
...
## Cost-efficient deployment — recommendations
...
|---|---|---|
| Frontend hosting | Vercel | $0 (Hobby) if usage stays personal/low-key, or **$20/seat (Pro)** — Vercel's Hobby tier is licensed for non-commercial use; an internal company tool used by paid staff is commercial use, so Pro is the compliant choice once this is a real team tool, not a side project. |
| Frontend cron jobs | Already daily-only (`vercel.json`: 05:00/06:00/07:00 UTC) | $0 extra — Hobby restricts cron to once-daily schedules anyway, and this app never needed more than that, so no upgrade is forced by cron alone. Per-project cron limit was raised to 100 on every plan in Jan 2026, well above the 3 this app uses. |
| Database | **Keep the existing Supabase project** | $0 (Free tier: 500MB DB, 1GB storage, 50k MAUs) until you outgrow it, then $25/mo (Pro: 8GB DB, 100GB storage). Don't provision a second Postgres for the NestJS backend — point its `DATABASE_URL` at the same Supabase Postgres instance via its direct connection string (Settings → Database → Connection string), through the **pooler** connection (PgBouncer, included free) rather than the direct one, to stay well within the 60-direct/200-pooled connection limit on the free tier. |
| NestJS backend hosting (compute only) | **Fly.io** (cheapest, sub-$2/mo for a small shared-CPU VM, pay-as-you-go) or **Railway** (simpler DX, $5/mo Hobby plan, usage-based) | $2–5/mo |
| Auth (if/when Clerk migration completes) | Clerk | $0 — free tier now covers 50,000 monthly active users (raised from 10k in 2026), comfortably covers an internal team + candidate logins for a long time. |
| Resume storage | SharePoint (your existing M365 subscription) | $0 incremental — Graph API calls aren't separately billed; storage comes out of your existing SharePoint quota. |

**Realistic total: $2–7/month** if you stay on free tiers where eligible (Supabase Free,
Clerk Free, Vercel Hobby if usage permits) plus cheap backend compute. **$25–32/month** if
you need Vercel Pro for commercial-use compliance and/or outgrow Supabase's free tier. Both
are inexpensive for what this app does — the actual cost driver to watch is Supabase
...
"just to get started" — reusing the Supabase Postgres connection string is both cheaper
...
- **First login on a fresh deploy**: no self-serve signup. Create the first user via
  Supabase's Admin API or dashboard, then log in at `/login` — the first successful login
  auto-promotes to `admin` if zero admins exist yet.
- **Scheduled jobs need `CRON_SECRET` set in Vercel**, or all three cron jobs 401 silently
  forever. Single highest-impact missing config right now.
- **`npm run build` clobbers `.next`**, which `npm run dev` also uses — `rm -rf .next` and
...
- **Migrations**: `supabase/migrations/*.sql`, applied in order via `npx supabase db push`.
  `sql/01_schema.sql` is a convenience snapshot, not the source of truth — the migrations
  folder is.
- **Backups**: daily JSON snapshot to Supabase Storage (needs `CRON_SECRET`), plus an
  on-demand download on `/ops`. A dump, not a restore tool.
...
already deliberately scoped out — the reasoning for each deferral is written down, not
just the decision.
